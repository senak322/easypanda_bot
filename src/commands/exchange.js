...
Без выходных`
  ...
  bot.command('contactUser', async (ctx) => {
    const args = ctx.message.text.split(' ').slice(1);
    if (args.length === 0) {
        return ctx.reply('Пожалуйста, укажите ID пользователя, которому нужно отправить сообщение. Например: /contactUser 123456789');
    }

    const userId = args[0]; // ID пользователя
    const message = 'Похоже, что из-за настроек вашей приватности мы не можем напрямую связаться с вами. Пожалуйста, напишите нашему администратору, нажав на кнопку ниже, чтобы мы могли ответить на ваши вопросы или помочь с заявкой.';

    try {
        await bot.telegram.sendMessage(userId, message, {
            reply_markup: {
                inline_keyboard: [[
                    { text: 'Написать администратору', url: 'https://t.me/easypandamoney' } // Замените на реальный username администратора
                ]]
            }
        });
...
bot.command("pending_orders", async (ctx) => {
  ...
  try {
      ...
      console.error("Ошибка при получении заявок, ожидающих подтверждения:", error);
      ctx.reply("Произошла ошибка при получении заявок, ожидающих подтверждения.");
  }
});
...
bot.command("close_order", async (ctx) => {
  ...
  const args = ctx.message.text.split(" ").slice(1);
  if (args.length === 0) {
      return ctx.reply("Пожалуйста, укажите хеш заявки. Например: /close_order ABC123");
  }
  ...
  try {
      ...
      bot.telegram.sendMessage(order.userId, `Ваша заявка #${hash} была закрыта администратором.`);
  } catch (error) {
